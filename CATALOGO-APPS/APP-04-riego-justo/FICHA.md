...
<h1 align="center">Riego Justo</h1>

<p align="center">
  Catálogo de dominios de ejemplo · <strong>App 04</strong> · Control de turnos de riego<br>
...
En la comisión de regantes el turno de agua se anota en un cuaderno. Los conflictos por minutos de riego son constantes y no hay forma de probar quién regó cuándo.

**Segmento objetivo.** Regantes de una comisión de 300 usuarios y el tomero que asigna los turnos
...
> El registro se hace **en la parcela, sin señal**. Exige funcionamiento sin conexión con sincronización posterior. Es el caso donde una web es directamente inviable.

**Capacidades del móvil que esta app usa de verdad.**

| Capacidad | Por qué la necesita |
|---|---|
| **Funcionamiento sin conexión con sincronización diferida** | En la parcela no hay señal. Si la app exige red, no sirve |
...
|---|---|---|
| **1** | 01–03 | Autenticación y padrón de regantes |
| **2** | 04–06 | Calendario de turnos |
| **3** | 07–09 | Registro de riego sin conexión |
| **4** | 10–12 | Sincronización y resolución de conflictos |
| **5** | 13–16 | Reportes de consumo e incidencias |
...
|---|---|
| **Regante** | Entidad central del dominio |
| **Parcela** | Entidad central del dominio |
| **Turno** | Entidad central del dominio |
| **Registro de riego** | Entidad central del dominio |
| **Incidencia** | Entidad central del dominio |
...
|---|---|
| Nombre del regante | Consentimiento previo, informado y expreso |
| Ubicación de la parcela | Consentimiento previo, informado y expreso |
| Horas de riego consumidas | Consentimiento previo, informado y expreso |
...
| Recurso | Métodos |
|---|---|
| `/regante` | `GET` · `POST` · `PUT` · `DELETE` |
| `/parcela` | `GET` · `POST` · `PUT` · `DELETE` |
| `/turno` | `GET` · `POST` · `PUT` · `DELETE` |
| `/registro-de-riego` | `GET` · `POST` · `PUT` · `DELETE` |
| `/incidencia` | `GET` · `POST` · `PUT` · `DELETE` |
